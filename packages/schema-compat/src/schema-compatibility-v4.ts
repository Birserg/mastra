...
export class SchemaCompatLayer {
  ...
  public defaultZodObjectHandler(
    ...
    // Only copy catchall if it's not ZodUnknown (passthrough), or if passthrough option is true
    if (value._zod.def.catchall && !(value._zod.def.catchall instanceof z.ZodNever)) {
      // Skip copying ZodUnknown catchall (passthrough) if options.passthrough is false
      if (!(value._zod.def.catchall instanceof z.ZodUnknown) || options.passthrough) {
  ...
  public defaultZodNullableHandler(
    ...
    if (handleTypes.includes(value.constructor.name as AllZodType)) {
      const processed = this.processZodType(value._zod.def.innerType);
